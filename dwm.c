...
setup(void)
{
	...
	XChangeProperty(dpy, wmcheckwin, netatom[NetWMName], utf8string, 8,
		PropModeReplace, (unsigned char *) "xvvm", 3);
...
spawn(const Arg *arg)
{
	...
	if( fork() == 0 ){
		...
		fprintf(stderr, "xvvm: execvp %s", ((char **)arg->v)[0]);
		perror(" failed");
		exit(EXIT_SUCCESS);
	}
}
...
xerror(Display *dpy, XErrorEvent *ee)
{
	...
	fprintf(stderr, "xvvm: fatal error: request code=%d, error code=%d\n",
...
{
	die("xvvm: another window manager is already running");
	return -1;
}
